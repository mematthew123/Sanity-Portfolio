            ...
            postData.map((post, index) => (
              <article>
                <Link to={"/post/" + post.slug.current} key={post.slug.current}>
                  <span
                    className="block h-64 relative rounded shadow leading-snug bg-white border-l-8 border-blue-500"
                    key={index}
                  >
                    <img
                    ...
                    <span className="block relative h-full justify-end items-end pr-4 pb-4">
                      <h3 className="text-white text-lg font-bold px-3 py-4 bg-black  bg-opacity-75 rounded">
                        {post.title}
                      </h3>
                    </span>
                  </span>
